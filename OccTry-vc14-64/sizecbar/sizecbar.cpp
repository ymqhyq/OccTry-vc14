...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    if (bHorz)
        return CSize(std::max(m_szMinHorz.cx, m_szHorz.cx),
            std::max(m_szMinHorz.cy, m_szHorz.cy));

    return CSize(std::max(m_szMinVert.cx, m_szVert.cx),
        std::max(m_szMinVert.cy, m_szVert.cy));
}
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
    ...
    m_szFloat.cx = std::max(m_szFloat.cx, m_szMinFloat.cx);
    m_szFloat.cy = std::max(m_szFloat.cy, m_szMinFloat.cy);
...
void CSizingControlBar::OnNcPaint()
{
    ...
    mdc.FillRect(rcDraw, CBrush::FromHandle(
        (HBRUSH) GetClassLong(m_hWnd, GCLP_HBRBACKGROUND)));//yxk20200104 GCL_HBRBACKGROUND->GCLP_HBRBACKGROUND
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
    {
...
void CSizingControlBar::StartTracking(UINT nHitTest, CPoint point)
{
    ...
    if (!IsSideTracking())
    {
        ...
        for (int i = 0; i < arrSCBars.GetSize(); i++)
            nMinWidth = std::max(nMinWidth, bHorz ? 
                (int)arrSCBars[i]->m_szMinHorz.cy :
                (int)arrSCBars[i]->m_szMinVert.cx);
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    nTrackPos = std::max(m_nTrackPosMin, min(m_nTrackPosMax, nTrackPos));
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (bIsSizingBar)
        {
            nLengthActual += bHorz ? pBar->m_szHorz.cx - 2 : 
                pBar->m_szVert.cy - 2;
            nWidthMax = std::max(nWidthMax, bHorz ? (int)pBar->m_szHorz.cy :
                (int)pBar->m_szVert.cx);
        }
        else
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = std::max(m_szMinHorz.cx, (long) pApp->GetProfileInt(
        szSection, _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = std::max(m_szMinHorz.cy, (long) pApp->GetProfileInt(
        szSection, _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = std::max(m_szMinVert.cx, (long) pApp->GetProfileInt(
        szSection, _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = std::max(m_szMinVert.cy, (long) pApp->GetProfileInt(
        szSection, _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = std::max(m_szMinFloat.cx, (long) pApp->GetProfileInt(
        szSection, _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = std::max(m_szMinFloat.cy, (long) pApp->GetProfileInt(
